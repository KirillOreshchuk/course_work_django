    ...
    def handle(self, *args, **options):
        ...
        for mailing in active_mailings:
            ...
            if mailing.start_time >= current_time:
                ...
                next_newsletter_str: str = mailing.next_newsletter.strftime('%Y-%m-%d %H:%M')
                if next_newsletter_str <= now:
                    if mailing.periodicity == "Раз в день":
                        mailing.next_newsletter = current_time + timedelta(days=1)
                        mailing.save()
                    elif mailing.periodicity == "Раз в неделю":
                        mailing.next_newsletter = current_time + timedelta(days=7)
                        mailing.save()
                    elif mailing.periodicity == "Раз в месяц":
                        today = datetime.today()
                        days = calendar.monthrange(today.year, today.month)[1]
                        mailing.next_newsletter = current_time + timedelta(days=days)
                        mailing.save()
                    if mailing.next_newsletter > mailing.end_time:
                        mailing.status = "Завершена"
                        mailing.save()
                    ...
                    try:
                        ...
                        log.save()
                elif mailing.next_newsletter >= mailing.end_time:
                    mailing.status = "Завершена"
                    mailing.save()
